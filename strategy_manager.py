...
STRATEGY_TEMPLATES = {
    "Quantum Scalper": StrategyConfig(
        ...
        vol_multiplier=1.2, min_confidence=62.0,
        time_filter=_tf_off(),
    ),
    "NEXUS SMC Elite": StrategyConfig(
        ...
        vol_multiplier=1.2, min_confidence=65.0,
        time_filter=_tf_off(),
    ),
    "Titan Trend": StrategyConfig(
        ...
        ema_fast=21, ema_medium=50, ema_slow=100,
        macd_fast=12, macd_slow=26, macd_signal=9,
        bb_period=20, bb_std=2.0, atr_period=14,
        vol_multiplier=1.2, min_confidence=65.0,
        time_filter=_tf_off(),
    ...
    "Iron Shield": StrategyConfig(
        ...
        macd_fast=12, macd_slow=26, macd_signal=9,
        bb_period=20, bb_std=2.0, atr_period=14,
        vol_multiplier=1.2, min_confidence=68.0,
        time_filter=_tf_off(),
    ),
}
...
class SignalGenerator:
    ...
    def generate(self, df: pd.DataFrame, cfg: StrategyConfig) -> dict:
        ...
        # Решение
        threshold = 3 if cfg.style in ["scalp","aggressive"] else 4

        total_votes = vl + vs
        if vl >= threshold and vl > vs:
            # Confidence: колко доминира long vs short + брой гласове
            dominance = vl / max(total_votes, 1)   # 0.5..1.0
            volume_bonus = min(vl / 10, 1.0) * 15  # до +15% за повече гласове
            conf = min(97, 55 + dominance * 30 + volume_bonus)
        ...
        if vs >= threshold and vs > vl:
            dominance = vs / max(total_votes, 1)
            volume_bonus = min(vs / 10, 1.0) * 15
            conf = min(97, 55 + dominance * 30 + volume_bonus)
            if conf >= cfg.min_confidence:
