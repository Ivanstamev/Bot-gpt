...
# ── KEEP-ALIVE: пингваме себе си на всеки 10 мин за да не заспим ──
def _keep_alive():
...
if st.session_state.running:
    now = time.time()
    last_tick_time = st.session_state.get("last_tick_time", 0)
    # Викаме tick само веднъж на 15 секунди
    if now - last_tick_time >= 15:
        result = bot.tick()
...
with tab_dash:
    # Auto-refresh на всеки 15с само докато бота е активен — без sleep блокиране
    if st.session_state.running:
        elapsed = time.time() - st.session_state.get("last_tick_time", 0)
        remaining = max(1, int(15 - elapsed))
        components.html(
            f'<script>setTimeout(function(){{window.parent.document.querySelector("[data-testid=stApp]").dispatchEvent(new Event("rerun"));}},{remaining*1000});</script>',
            height=0,
        )
    ...
    if st.session_state.running:
        ...
        st.caption(f"⏱ Следващ тик след {remaining:.0f}с — бот активен")
        # Само ако е изтекло — презареждаме
        if remaining < 1:
            st.rerun()
