...
with tab_chart:
    ...
    # TradingView widget — components.html за да работят скриптовете
    components.html(f"""<!DOCTYPE html><html><head>
    <style>body{{margin:0;padding:0;background:#040810;}}
    .tv-container{{border:1px solid #162035;border-radius:16px;overflow:hidden;}}</style>
    </head><body>
    ...
    new TradingView.widget({{
      ...
      "symbol":"{tv_symbol}","interval":"{tv_tf}",
      "timezone":"Etc/UTC","theme":"{tv_theme}","style":"1","locale":"en",
      "toolbar_bg":"#0d1626","enable_publishing":false,
      "hide_side_toolbar":false,"allow_symbol_change":true,
      "studies":["RSI@tv-basicstudies","MACD@tv-basicstudies","BB@tv-basicstudies"],
      "container_id":"tradingview_nexus","show_popup_button":true
    }});
    </script></div></div></body></html>""", height=520, scrolling=False)
...
with tab_demo:
    ...
    with st.expander("💰 Задай демо баланс", expanded=True):
        ...
        custom_bal = c1.number_input("Ръчна сума (USDT)", 5.0, 1_000_000.0,
                                     float(st.session_state.demo_balance), 5.0, key="custom_bal_inp")
...
with tab_bt:
    ...
    if st.button("🚀 СТАРТИРАЙ BACKTEST", key="run_bt"):
        with st.spinner(f"Изпълнява се backtest {bt_days} дни за {bt_strat}..."):
            # Запази активната стратегия и я възстанови след бектеста
            prev_strategy = bot.strategy._active_name
            bot.select_strategy(bt_strat)
            bot.risk.update_config(leverage=bt_lev)
            # Нулирай daily stats за да не блокира бектеста
            from risk_management import DailyStats
            bot.risk.daily = DailyStats()
            result = bot.run_backtest(days=bt_days)
            # Върни предишната стратегия
...
# Safety fallback
try:
    if 'df' in locals() and df is not None and df.empty:
